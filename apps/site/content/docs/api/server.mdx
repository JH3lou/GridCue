...
They are documented on the [Providers](/docs/api/providers#createjevprovider) page. `createJevProvider` uses `@typesafe-ai/sdk`, an optional peer dependency: install it on the server when you use Jev. It loads on the first Jev request, so `gridcue/server` works without it for the Mock or your own provider; a Jev request without it fails with `INPUT_CONFIG`.
