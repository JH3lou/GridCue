...
| Compound requests ("Roth IRAs grouped by rep") | Can propose a wrong view | Handled |
| "Also group by advisor" | Can propose a wrong view | Handled |
| Nesting ("advisor within custodian") | Can propose a wrong view | Handled |

Question count grows with your columns and enum values, so your numbers will differ.

Focused is not simply "fan-out, cheaper". It gets simple, single-change requests right, but it still proposes wrong views on the request types the first version never handled: nesting, "also", and several changes in one part. Choose it only when your Users' requests are simple, and confirm that with evals on your own requests.
