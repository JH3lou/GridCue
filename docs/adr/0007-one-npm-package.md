...
GridCue must work with the tables people already use in Vite and Next.js apps. The first release ships two Grid Adapters: a built-in Rows Adapter that works with any table, and a TanStack Table v9 adapter, which also covers shadcn's Data Table. TanStack Start, the framework, is out of scope.
